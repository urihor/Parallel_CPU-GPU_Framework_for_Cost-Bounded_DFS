...
#ifndef PDB_BITS
#define PDB_BITS 8 // 8 for nibble-packed (values saturate at 15), or 8 for 1 byte/entry
#endif
...
bool save(const std::string& path, bool with_progress = true) const; // raw dump (packed)
bool save_atomic(const std::string& path, bool with_progress = true) const; // write to path.tmp then rename
